...
class MarsFrameworkTests: XCTestCase {
    ...
    func testGridSize() {
        let numbersOfResults = makeSUT()?.matrix.gridSize
    ...
    func testValues() {
        XCTAssertEqual(makeSUT()?.matrix.values, [4,2,3,2,2,1,3,2,1])
    }
    ...
    func testRowCollValues() {
        // [(0,0): (0,1); (1,2)]
        XCTAssertEqual(makeSUT()?.matrix.matixIndex(at: 0), Matrix.MatrixIndex(row: 0, col: 0))
        XCTAssertEqual(makeSUT()?.matrix.matixIndex(at: 2), Matrix.MatrixIndex(row: 0, col: 2))
        XCTAssertEqual(makeSUT()?.matrix.matixIndex(at: 3), Matrix.MatrixIndex(row: 1, col: 0))

        XCTAssertEqual(makeSUT()?.matrix.matixIndex(at: 5), Matrix.MatrixIndex(row: 1, col: 2))
        XCTAssertEqual(makeSUT()?.matrix.matixIndex(at: 8), Matrix.MatrixIndex(row: 2, col: 2))
    }

    func test_calculateCellValueByMatrixIndex() {
        let index = 1
        let matrix = makeSUT()?.matrix.matixIndex(at: index)

        let value = makeSUT()?.matrix.calculateCell(matrixIndex: matrix)
